...
# People Search Claude Skill (Exa)

Search for **people and their public social profiles** using Exa MCP’s `people_search_exa` tool.

This pairs well with recruiting, sales, expert-finding, and market research workflows.
...
```
Step 1: Install or update Exa MCP (people_search_exa only)

Note: `people_search_exa` is disabled by default in Exa MCP, so you must explicitly enable it via the `tools=` parameter.
...
claude mcp add --transport http exa "https://mcp.exa.ai/mcp?tools=people_search_exa"
