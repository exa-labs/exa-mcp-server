...
|---------|--------|
| **URL Parameters** | |
| `?tools=web_search_exa,crawling_exa` | ✅ Works identically |
| `?exaApiKey=YOUR_KEY` | ✅ Works identically |
| `?debug=true` | ✅ Works identically |
| **Default Tools** | |
| `web_search_exa` + `crawling_exa` enabled by default | ✅ Preserved |
...
https://your-project.vercel.app/api/mcp?tools=web_search_exa,crawling_exa
...
### Local Testing
```bash
# Test default (should enable web_search_exa + crawling_exa)
...
| `ENABLED_TOOLS` | No | Comma-separated tool list | `web_search_exa,crawling_exa` |
...
- [x] Default tools are `web_search_exa` + `crawling_exa`
