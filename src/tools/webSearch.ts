...
Note: This tool does NOT support deep search. For deep search with query expansion and synthesized answers, use the deep_search_exa tool instead.`,
    {
      query: z.preprocess(v => typeof v === 'number' || typeof v === 'boolean' ? String(v) : v, z.string()).describe("Websearch query"),
      ...
      livecrawl: z.enum(['fallback', 'preferred']).optional().catch(undefined).describe("Live crawl mode - 'fallback': use live crawling as backup if cached content unavailable, 'preferred': prioritize live crawling (default: 'fallback')"),
      type: z.enum(['auto', 'fast']).optional().catch(undefined).describe("Search type - 'auto': balanced search (default), 'fast': quick results"),
      category: z.enum(['company', 'research paper', 'people']).optional().catch(undefined).describe("Filter results to a specific category. Must be one of: 'company', 'research paper', 'people'. Omit if no specific category is needed."),
      contextMaxCharacters: z.coerce.number().optional().catch(undefined).describe("Maximum characters for context string optimized for LLMs (must be a number, default: 10000)")
    },
    {
      readOnlyHint: true,
